...
minmax typ n drzewo
     | n == 1 = minimum (funkcjaOceniajacaDzieci typ drzewo)
     | mod n 2 == 0 = maximum (map (\x -> minmax typ (n-1) x) (wezDzieciDrzewa drzewo)) 
     | otherwise = minimum (map (\x -> minmax typ (n-1) x) (wezDzieciDrzewa drzewo))
     


wywolajMinMax typ drzewo = (wezDzieci drzewo)!!(fromJust $ elemIndex (maximum lista) lista) 
     where lista = (map (\x -> (minmax typ 3 x)) (wezDzieciDrzewa drzewo))
